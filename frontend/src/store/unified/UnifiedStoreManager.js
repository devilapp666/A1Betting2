...
export const useUnifiedStore = create()(devtools(persist((set, get) => ({
    // Initial State
    predictions: {},
    latestPredictions: [],
    betting: {
    ...
    actions: {
        ...
        updateBetStatus: (betId, status) => {
            set((state) => {
                const updatedBets = state.betting.bets.map((bet) => bet.id === betId ? { ...bet, status } : bet);
